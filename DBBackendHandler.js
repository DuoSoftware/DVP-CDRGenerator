...
var getCallLeg = function(uuid, callback)
{
    try
    {
        dbModel.CallCDR.find({where :[{$or: [{Uuid: uuid, Direction: 'inbound', ObjCategory: {ne: 'CONFERENCE'}, $or: [{OriginatedLegs: {ne: null}}, {OriginatedLegs: null, $or:[{ObjType: 'HTTAPI'},{ObjType: 'SOCKET'},{ObjType: 'REJECTED'},{ObjType: 'FAX_INBOUND'},{ObjCategory: 'DND'},{ObjCategory: 'OUTBOUND_DENIED'}]}]},{Uuid: uuid, ObjCategory: 'DIALER'}]}]}).then(function(callLeg)
        {
